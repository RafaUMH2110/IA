...
@dataclass
class PdfjsConfig:
    """Configuración de pdf.js, que a diferencia del resto de librerías
    necesita DOS URLs: la de la librería y la de su "worker" (el script que
    procesa los PDF en un hilo aparte)."""
    activa: bool
    url: str
    worker_url: str
...
class LibreriasConfig:
    ...
    mammoth: LibreriaConfig
    pdfjs: PdfjsConfig


@dataclass
class AdjuntosConfig:
    """Documentos de referencia que la persona puede adjuntar a su consulta
    para que la IA los tenga en cuenta al generar la respuesta final."""
    activo: bool
    formatos_permitidos: list[str]
    tamano_maximo_mb: int
    caracteres_maximos_totales: int
    etiqueta_campo: str
    ayuda_campo: str
    boton_elegir_archivos: str


@dataclass
class GuardarPromptsConfig:
    """Opción para guardar en un único archivo .txt el "Rol del usuario" y
    la "Consulta" tal como los ha escrito la persona, independientemente
    de la consulta a la IA."""
    activo: bool
    etiqueta_checkbox: str
    etiqueta_nombre_archivo: str
    ayuda_nombre_archivo: str
    nombre_archivo_por_defecto: str
    boton_guardar: str
...
class ConfiguracionApp:
    ...
    adjuntos: AdjuntosConfig
    guardar_prompts: GuardarPromptsConfig
...
def _construir_ui(datos: dict[str, Any]) -> TextosUI:
    ...
    return TextosUI(
        ...
        mensaje_truncada=str(_obtener(
            b, "mensaje_truncada",
            "Respuesta generada, pero truncada por el límite de tokens (ver aviso junto al resultado).",
        )),
        aviso_truncado=str(_obtener(
            b, "aviso_truncado",
            "Respuesta truncada: se ha interrumpido porque se ha alcanzado el límite de tokens "
            "disponible («Número de tokens» en la configuración). Auméntalo o simplifica la "
            "consulta para obtener una respuesta completa.",
        )),
...
def _construir_tema(datos: dict[str, Any]) -> TemaConfig:
    ...
    return TemaConfig(
        ...
        alto_maximo_resultado=str(_obtener(b, "alto_maximo_resultado", "480px")),
    )
...
def _construir_pdfjs(bruto: dict[str, Any]) -> PdfjsConfig:
    return PdfjsConfig(
        activa=bool(_obtener(bruto, "activa", True)),
        url=str(_obtener(bruto, "url", "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.11.338/pdf.min.js")),
        worker_url=str(_obtener(bruto, "worker_url", "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.11.338/pdf.worker.min.js")),
    )
...
def _construir_librerias(datos: dict[str, Any]) -> LibreriasConfig:
    ...
    return LibreriasConfig(
        ...
        mammoth=_construir_libreria(
            _obtener(b, "mammoth", {}), "https://cdn.jsdelivr.net/npm/mammoth@1.9.0/mammoth.browser.min.js"
        ),
        pdfjs=_construir_pdfjs(_obtener(b, "pdfjs", {})),
    )


def _construir_adjuntos(datos: dict[str, Any]) -> AdjuntosConfig:
    b = _obtener(datos, "adjuntos", {})
    formatos = _obtener(b, "formatos_permitidos", [".txt", ".md", ".markdown", ".docx", ".pdf"])
    return AdjuntosConfig(
        activo=bool(_obtener(b, "activo", True)),
        formatos_permitidos=[str(f) for f in formatos],
        tamano_maximo_mb=int(_obtener(b, "tamano_maximo_mb", 10)),
        caracteres_maximos_totales=int(_obtener(b, "caracteres_maximos_totales", 60000)),
        etiqueta_campo=str(_obtener(b, "etiqueta_campo", "Documentos de referencia (opcional)")),
        ayuda_campo=str(_obtener(b, "ayuda_campo", "")),
        boton_elegir_archivos=str(_obtener(b, "boton_elegir_archivos", "Elegir archivos…")),
    )


def _construir_guardar_prompts(datos: dict[str, Any]) -> GuardarPromptsConfig:
    b = _obtener(datos, "guardar_prompts", {})
    return GuardarPromptsConfig(
        activo=bool(_obtener(b, "activo", True)),
        etiqueta_checkbox=str(_obtener(b, "etiqueta_checkbox", "Guardar los prompts en un archivo de texto")),
        etiqueta_nombre_archivo=str(_obtener(b, "etiqueta_nombre_archivo", "Nombre del archivo")),
        ayuda_nombre_archivo=str(_obtener(
            b, "ayuda_nombre_archivo",
            "Se guardará como .txt, con el «Rol del usuario» y la «Consulta» tal como los has escrito.",
        )),
        nombre_archivo_por_defecto=str(_obtener(b, "nombre_archivo_por_defecto", "prompts")),
        boton_guardar=str(_obtener(b, "boton_guardar", "Guardar prompts (.txt)")),
    )
...
def construir_configuracion(datos: dict[str, Any]) -> ConfiguracionApp:
    ...
    return ConfiguracionApp(
        ...
        adjuntos=_construir_adjuntos(datos),
        guardar_prompts=_construir_guardar_prompts(datos),
    )
