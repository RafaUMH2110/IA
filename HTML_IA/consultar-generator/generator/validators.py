...
_RE_EXTENSION_ARCHIVO = re.compile(r"^\.[a-z0-9]+$")
...
def validar_configuracion(config: ConfiguracionApp) -> list[str]:
    ...
    _validar_adjuntos(config, errores)
    _validar_guardar_prompts(config, errores)
...
def _validar_tema(config: ConfiguracionApp, errores: list[str]) -> None:
    ...
    if not _RE_MEDIDA_CSS.match(tema.alto_maximo_resultado):
        errores.append(f"tema.alto_maximo_resultado debe ser una medida CSS simple, p. ej. '480px' (valor actual: '{tema.alto_maximo_resultado}').")
...
def _validar_librerias(config: ConfiguracionApp, errores: list[str], advertencias: list[str]) -> None:
    ...
    if config.adjuntos.activo and ".docx" in config.adjuntos.formatos_permitidos and not librerias.mammoth.activa:
        errores.append("adjuntos.formatos_permitidos incluye '.docx' pero librerias.mammoth.activa es 'false'.")
    if config.adjuntos.activo and ".pdf" in config.adjuntos.formatos_permitidos and not librerias.pdfjs.activa:
        errores.append("adjuntos.formatos_permitidos incluye '.pdf' pero librerias.pdfjs.activa es 'false'.")
    ...
    for nombre, libreria in (
        ...
        ("librerias.mammoth", librerias.mammoth),
        ("librerias.pdfjs", librerias.pdfjs),
    ):
    ...
    if librerias.pdfjs.activa and not _RE_URL.match(librerias.pdfjs.worker_url):
        errores.append(f"librerias.pdfjs.worker_url no es una URL http(s) válida: '{librerias.pdfjs.worker_url}'.")


def _validar_adjuntos(config: ConfiguracionApp, errores: list[str]) -> None:
    adjuntos = config.adjuntos
    if not adjuntos.activo:
        return

    if not adjuntos.formatos_permitidos:
        errores.append("adjuntos.formatos_permitidos no puede estar vacío si adjuntos.activo es 'true'.")
    for formato in adjuntos.formatos_permitidos:
        if not _RE_EXTENSION_ARCHIVO.match(formato):
            errores.append(
                f"adjuntos.formatos_permitidos contiene un valor no válido: '{formato}' "
                f"(debe ser una extensión en minúsculas con el punto, p. ej. '.pdf')."
            )

    formatos_soportados = {".txt", ".md", ".markdown", ".docx", ".pdf"}
    no_soportados = set(adjuntos.formatos_permitidos) - formatos_soportados
    if no_soportados:
        errores.append(
            f"adjuntos.formatos_permitidos incluye formato(s) que la aplicación no sabe extraer: "
            f"{sorted(no_soportados)} (soportados: {sorted(formatos_soportados)})."
        )

    if adjuntos.tamano_maximo_mb <= 0:
        errores.append("adjuntos.tamano_maximo_mb debe ser mayor que 0.")
    if adjuntos.caracteres_maximos_totales <= 0:
        errores.append("adjuntos.caracteres_maximos_totales debe ser mayor que 0.")


def _validar_guardar_prompts(config: ConfiguracionApp, errores: list[str]) -> None:
    gp = config.guardar_prompts
    if not gp.activo:
        return
    if not gp.nombre_archivo_por_defecto.strip():
        errores.append("guardar_prompts.nombre_archivo_por_defecto no puede estar vacío.")
    elif not _RE_NOMBRE_ARCHIVO.match(gp.nombre_archivo_por_defecto):
        errores.append(f"guardar_prompts.nombre_archivo_por_defecto contiene caracteres no válidos: '{gp.nombre_archivo_por_defecto}'.")
    if not gp.etiqueta_checkbox.strip():
        errores.append("guardar_prompts.etiqueta_checkbox no puede estar vacío.")
    if not gp.boton_guardar.strip():
        errores.append("guardar_prompts.boton_guardar no puede estar vacío.")
