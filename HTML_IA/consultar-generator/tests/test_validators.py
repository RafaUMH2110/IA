...
class TestErroresDetectados(unittest.TestCase):
    ...
    def test_adjuntos_formato_no_soportado(self) -> None:
        self._esperar_error({"adjuntos": {"formatos_permitidos": [".exe"]}}, "no sabe extraer")

    def test_adjuntos_pdf_sin_libreria_pdfjs(self) -> None:
        self._esperar_error(
            {"adjuntos": {"formatos_permitidos": [".pdf"]}, "librerias": {"pdfjs": {"activa": False}}},
            "librerias.pdfjs",
        )

    def test_adjuntos_docx_sin_libreria_mammoth(self) -> None:
        self._esperar_error(
            {"adjuntos": {"formatos_permitidos": [".docx"]}, "librerias": {"mammoth": {"activa": False}}},
            "librerias.mammoth",
        )

    def test_guardar_prompts_nombre_archivo_invalido(self) -> None:
        self._esperar_error({"guardar_prompts": {"nombre_archivo_por_defecto": "a/b"}}, "nombre_archivo_por_defecto")

    def test_guardar_prompts_etiqueta_vacia(self) -> None:
        self._esperar_error({"guardar_prompts": {"etiqueta_checkbox": ""}}, "etiqueta_checkbox")
