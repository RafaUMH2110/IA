...
tipográficas, radio de borde, sombra, ancho máximo de página y alto máximo
del panel de "Resultado" (`alto_maximo_resultado`; a partir de esa altura
aparece una barra de desplazamiento en lugar de mostrar la respuesta
completa). El tema por defecto usa una paleta de azules muy claros con
blanco y gris claro, tal y como se pidió originalmente.
...
URLs (y activación) de las seis dependencias JavaScript cargadas desde
CDN: `marked` (Markdown → HTML), `sanitizador` (DOMPurify), `docx` (Word),
`jspdf` (PDF), `mammoth` (extraer texto de .docx adjuntados) y `pdfjs`
(extraer texto de .pdf adjuntados; necesita además `worker_url`, el script
que pdf.js ejecuta en un hilo aparte). `markdown` y `sanitizador` son
obligatorias. Las demás solo son necesarias si el formato correspondiente
está activo en `exports` o en `adjuntos.formatos_permitidos` — si las
desactivas, el propio generador omite su etiqueta `<script>` del HTML
final para no cargar peso innecesario.

### 10. `adjuntos`

Permite a la persona que use la aplicación subir documentos de referencia
(.txt, .md, .docx, .pdf) junto a su consulta, para que la IA los tenga en
cuenta al generar la respuesta. El texto se extrae **enteramente en el
navegador** (nunca se sube a ningún servidor salvo la propia API de
Anthropic, como parte del mensaje final) y se añade a la plantilla
`prompts.final.user` mediante el marcador `{{documentos_adjuntos}}` — ver
la sección [Plantillas de prompts](#plantillas-de-prompts).

| Clave | Descripción |
|---|---|
| `activo` | Si es `false`, el campo de adjuntos no aparece en absoluto y no se cargan `mammoth` ni `pdfjs`. |
| `formatos_permitidos` | Lista de extensiones admitidas. Solo se admiten `.txt`, `.md`, `.markdown`, `.docx` y `.pdf` (son los únicos que la aplicación sabe extraer). |
| `tamano_maximo_mb` | Tamaño máximo por archivo individual. |
| `caracteres_maximos_totales` | Límite conjunto de texto extraído de todos los adjuntos combinados; si se supera, el contenido se recorta (o se omite un archivo completo) para no disparar el consumo de tokens de la consulta final. |
| `etiqueta_campo`, `ayuda_campo`, `boton_elegir_archivos` | Textos de la interfaz para este campo. |

### 11. `guardar_prompts`

Añade una casilla independiente de la consulta a la IA: al activarla, la
persona puede indicar un nombre de archivo y, pulsando su propio botón
("Guardar prompts (.txt)", junto al de "Consultar"), descargar un único
archivo `.txt` con el "Rol del usuario" y la "Consulta" tal como los ha
escrito — sin llamar a la API ni depender de haber consultado antes.

| Clave | Descripción |
|---|---|
| `activo` | Si es `false`, la casilla y el botón no aparecen en absoluto. |
| `etiqueta_checkbox` | Texto de la casilla. |
| `etiqueta_nombre_archivo`, `ayuda_nombre_archivo` | Etiqueta y ayuda del campo de nombre de archivo. |
| `nombre_archivo_por_defecto` | Valor inicial de ese campo (sin extensión; se añade `.txt` automáticamente). |
| `boton_guardar` | Texto del botón. |
...
| `{{documentos_adjuntos}}` | El texto extraído de los documentos de referencia adjuntados (ver sección `adjuntos`), o una cadena vacía si no se ha adjuntado ninguno. |
...
1. **Entrada del usuario.** La persona escribe su clave de API, elige un
   ...
   prompt de la respuesta final), su "Consulta" y, opcionalmente, adjunta
   documentos de referencia (.txt, .md, .docx, .pdf).
2. **Extracción de los documentos adjuntos (opcional).** Cada archivo se
   procesa en el propio navegador en cuanto se selecciona: texto plano
   para .txt/.md, `mammoth.js` para .docx y `pdf.js` para .pdf. El botón
   "Consultar" espera a que termine la extracción de todos los archivos.
3. **Primera consulta a la IA (mejora).** Al pulsar "Consultar", la
...
4. **Mejora del prompt.** Ese resultado se usa, sin mostrarlo, como
   entrada del siguiente paso.
5. **Segunda consulta a la IA (final).** La aplicación rellena la
   plantilla `prompts.final` con la consulta mejorada, el "Rol del
   usuario" y el texto extraído de los documentos adjuntos, y la envía a
   la API con el modelo y el número de tokens elegidos en pantalla.
6. **Renderizado en Markdown.** La respuesta final se interpreta como
   ...
   "Resultado".
7. **Exportación (opcional).** Si se activó "Exportar resultado", el botón
