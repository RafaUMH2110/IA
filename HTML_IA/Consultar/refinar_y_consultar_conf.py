...
# de "razonamiento adaptativo" (Sonnet 5, Opus 5, Fable 5.1...) fuerzan
...
MODELOS_DISPONIBLES: List[InfoModelo] = [
    ...
    InfoModelo(
        id_api="claude-opus-5",
        nombre="Claude Opus 5",
        contexto_maximo=1_000_000,
        max_output_tokens=128_000,
        admite_parametros_muestreo=False,
        notas=(
            "Modelo más potente de la generación 5, para razonamiento y "
            "agentes complejos. Mismo comportamiento que Sonnet 5 respecto "
            "a los parámetros de muestreo (no personalizables)."
        ),
    ),
    InfoModelo(
        id_api="claude-fable-5-1",
        nombre="Claude Fable 5.1",
        contexto_maximo=1_000_000,
        max_output_tokens=128_000,
        admite_parametros_muestreo=False,
        notas=(
            "Modelo de nivel superior a Opus, para el trabajo de "
            "conocimiento e ingeniería más exigente. Tampoco admite "
            "parámetros de muestreo personalizados."
        ),
    ),
    InfoModelo(
    ...
    InfoModelo(
        ...
        max_output_tokens=64_000,
        admite_parametros_muestreo=True,
        notas="Generación anterior a Opus 5. Admite parámetros de muestreo estándar.",
    ),
    InfoModelo(
...
class ConfiguracionGeneral:
    ...
    config_modelo_refinado: ConfiguracionModelo = field(
        default_factory=lambda: ConfiguracionModelo(
            indice_modelo=3,  # [3] Claude Haiku 4.5 -> rápido, económico y admite temperature baja.
            max_tokens=1024,
    ...
    config_modelo_principal: ConfiguracionModelo = field(
        default_factory=lambda: ConfiguracionModelo(
            indice_modelo=0,  # [0] Claude Sonnet 5 -> modelo principal por defecto.
            max_tokens=4096,
            temperature=0.7,  # Se ignorará automáticamente en Sonnet 5 (ver InfoModelo.admite_parametros_muestreo).
        )
    )
...
def _construir_parametros_llamada(config: ConfiguracionModelo) -> dict:
    ...
    if config.temperature is not None:
        parametros["temperature"] = config.temperature
